...
def create_empty_json_instance(img_file_path):
    '''
    :param img_file_path: img文件绝对路径
...
def extract_xys(axiss):
    ...
    '''
    return [float(axis.text) for axis in axiss if axis.tag != 'points']
...
def json_to_instance(json_file_path):
    ...
    _instance_clean(instance)
    return instance

def _instance_clean(instance):
    '''
    :param instance: labelme json instance
    ...
    for obj in instance['shapes'][:]:
        points = obj['points']
        if len(points) == 0: instance['shapes'].remove(obj)
...
def obj_in_crop(obj, crop_size, iou_thres=0.2):
    '''
    :param points: labelme json中一个obj
    :param crop_size: crop范围[上，下，左，右]
...
def obj_over_crop(obj, crop_size):
    '''
    :param points: labelme json中一个obj
    :param crop_size: crop范围[上，下，左，右]
    :return: 目标是否覆盖crop区域
    ...
    obj_box = Box(x, y, w, h)
    inter_area = calculate_inter_area(obj_box, crop_box)
    return inter_area != 0 and inter_area/crop_box.get_area() >= 0.99
